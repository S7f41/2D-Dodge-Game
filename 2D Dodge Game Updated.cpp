...
float velocity = 0.02;
float obstaclePenalty = 0.5;
float enemySpeedMultiplier = 1.5;
...
void drawObstacle(float left, float right, float top, float bottom)
{
...
void display(void)
{
    ...
    {
        drawObstacle(obstacles[i].left, obstacles[i].right, obstacles[i].top, obstacles[i].bottom);
    }
...
void animate()
{
    if (!isGameFinished)
    {
        ...
        for (int i = 0; i < sizeof(obstacles) / sizeof(obstacles[0]); ++i)
        {
            if (centerA1.x > obstacles[i].left && centerA1.x < obstacles[i].right &&
                    centerA1.y > obstacles[i].bottom && centerA1.y < obstacles[i].top)
            {
                velocity *= obstaclePenalty;
                centerE1.y -= velocity * enemySpeedMultiplier;
            }

            if (centerA2.x > obstacles[i].left && centerA2.x < obstacles[i].right &&
                    centerA2.y > obstacles[i].bottom && centerA2.y < obstacles[i].top)
            {
                velocity *= obstaclePenalty;
                centerE2.y -= velocity * enemySpeedMultiplier;
            }
        }
    }
    else
